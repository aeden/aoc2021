...
int main (int argc, char** argv) {
  ...
  /* 
  cout << "Diagram lines: " << endl;
  for (int i = 0; i < diagram.lines.size(); i++) {
    cout << diagram.lines[i].to_string() << endl;
    if (diagram.lines[i].is_45_diagonal()) {
      cout << "  45 diagonal" << endl;
      vector<Coordinates> points = diagram.lines[i].points;
      cout << "  has " << points.size() << " points" << endl;
      for (int j = 0; j < points.size(); j++) {
        cout << "  " << points[j].to_string() << endl;
      }
    }
  }
  */
